...
class VT100:
	...
	def lastLineChange(self):
		...
		print self.old_last_line.decode('big5', 'ignore')
		return ret
	
	def myShow(self):
		for Y in range(24):
			for X in range(80):
				sys.stdout.write(ord(self.lines[Y][X]))
				sys.stdout.write(', ')
			print
	...
	def show(self):
		...
		for line in self.lines:
			...
			#tmp = line[:i].tostring()
			tmp = line.tostring().decode('big5', 'ignore')
			print tmp
...
##			self.msg('found ESC [ %s %s' % (`argstr`, `c`))
	...
	# - re: desired contents given by a regular expression,
